...
caption step, and that step is optional — with Ollama absent, `media-describe`
...
- **`media-describe` writes into your images.** It adds `XMP:Description`,
...
- **`media-fix-extension` renames files.** Guarded: it only rewrites an extension when
...
- **`media-transcode` replaces the input by default.** The re-encode is
